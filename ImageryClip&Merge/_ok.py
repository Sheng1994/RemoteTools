...
for root, dirs, files in os.walk(file_dir):
    for file in tqdm(files[:2]):
        ...
        row_end= file.split('_')[2] #file.split('_')[4]('.')[0]
        col_end = file.split('_')[4].split('.')[0]
        xsize = int(col_end) - int(col_begin)
        ysize = int(row_end) - int(row_begin)
        ...
        out_ds.WriteRaster(xoff=int(col_begin), yoff=int(row_begin),xsize = xsize, ysize=ysize,buf_string = patch_ds,)
        # out_band2.WriteArray(patch_ds.GetRasterBand(1).ReadAsArray(), int(col_begin), int(row_begin))
        # out_band3.WriteArray(patch_ds.GetRasterBand(1).ReadAsArray(), int(col_begin), int(row_begin))
